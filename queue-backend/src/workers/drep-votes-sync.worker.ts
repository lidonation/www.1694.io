  ...
  async process(job: Job<DrepVotesSyncJobData, DrepVotesSyncJobResponse>): Promise<DrepVotesSyncJobResponse> {
    ...
    try {
      ...
      for (const d of dreps) {
        try {
          let page = 1, hasMore = true, synced = 0;

          while (hasMore) {
            try {
              const data = await this.blockfrostService.getDRepVotes(d.drepId, page, 100, 'desc');
              if (!data || data.length === 0) break;

              let existingCount = 0;
              for (const v of data) {
                try {
                  const ext = await votesRepo.findOne({ where: { txHash: v.tx_hash, certIndex: v.cert_index, voter: d.drepId } });
                  if (ext?.blockTime) {
                    existingCount++;
                    continue; 
                  }
                  ...
                  try {
                    const tx = await this.blockfrostService.getTransaction(v.tx_hash);
                  ...
                  this.logger.warn(`Vote upsert failed (${d.drepId}): ${e.message}`);
                }
              }
              ...
              this.logger.warn(`Votes page ${page} failed (${d.drepId}): ${e.message}`);
              hasMore = false;
            }
          }
          ...
          if (synced > 0) {
             ...
             await drepsRepo.update({ drepId: d.drepId }, { governanceVoteCount: count });
          }

          processed++;
          if (processed % 50 === 0) this.logger.log(`Processed ${processed} DReps...`);
          await new Promise(r => setTimeout(r, 100));
        } catch (e) {
      ...
      return { success: true, message: 'Synced DRep votes', drepVotesCount: total, processedDreps: processed };
    } catch (e) {

...
export class DrepClaimWorker extends WorkerHost {
  ...
  async process(job: Job<DRepClaimJobData, DRepClaimJobResponse>) {
    try {
      ...
      const registeredDrep = await this.voltaireService
        ...
        .where('del.stakeAddress = :stakeKey OR d.paymentAddress = :stakeKey', {
          stakeKey,
        })
      ...
      if (!registeredDrep)
        return {
          success: true,
          message: `No active DRep for ${stakeKey}`,
          isRegistered: false,
        };

      const claimDto: DRepClaimJobResponse = {
        success: true,
        message: `Data for ${stakeKey} processed`,
        isRegistered: true,
        stakeKey,
      ...
      return {
        success: true,
        message: 'DRep claimed',
        isRegistered: true,
        stakeKey,
        view: response.view,
        drepId: response.drepId,
      };
    } catch (e: any) {
  ...
  async autoClaimDRep(claim: DRepClaimJobResponse) {
    ...
    const existing = await this.voltaireService
      .createQueryBuilder()
      .select(['d.*', 's.*'])
      .from('drep', 'd')
      .leftJoin('signature', 's', 's."drepId" = d.id')
      .where('s."stakeKey" = :stakeKey', { stakeKey })
      .getRawOne();
    ...
    if (existing)
      return {
        ...
        isRegistered: true,
        stakeKey,
        view: existing.drep_bech32,
      };

    const dto: createDrepDto = {
      voter_id: view,
      drep_bech32: view,
      stake_addr: stakeKey,
      signatures: [
        {
          key: claim.signatureKey,
  ...
  async registerDrep(dto: createDrepDto) {
    return await this.voltaireService.transaction(async (manager) => {
      ...
      const sig = {
        stakeKey: dto.stake_addr,
        signature: dto.signatures[0].signature,
        signatureKey: dto.signatures[0].key,
        drep: drepId,
        voterId: dto.voter_id,
        drep_bech32: dto.drep_bech32,
        type: dto.signatures[0].type,
      };
      ...
      const existingSig = await manager
        ...
        .from('signature', 's')
        .leftJoin('drep', 'd', 'd.id = s."drepId"')
        .where('s."stakeKey" = :sK AND s."signatureKey" = :sQ', {
          sK: sig.stakeKey,
          sQ: sig.signatureKey,
        })
        .getRawOne();
      ...
      if (existingSig) {
        if (!existingSig.d_id) {
          const updated = await manager
            .createQueryBuilder()
            .update('signature')
            .set({ drepId, voterId: sig.voterId, drep_bech32: sig.drep_bech32 })
        ...
        const insertedSig = await manager
          ...
          .into('signature')
          .values({
            stakeKey: sig.stakeKey,
            signature: sig.signature,
            signatureKey: sig.signatureKey,
            drepId,
            voterId: sig.voterId,
            drep_bech32: sig.drep_bech32 || '',
            type: sig.type,

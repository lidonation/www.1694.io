...
export class CardanoMeshProvider implements AuthenticationProvider {
  ...
  async connect(walletName: string, params?: any): Promise<AuthResult> {
    try {
      ...
      if (network !== CONFIGURED_NETWORK_ID) {
        const networkName =
          CONFIGURED_NETWORK_ID === 1 ? 'Mainnet' : 'Testnet/Preview';
        throw new Error(
          `Wallet network mismatch. Please switch your wallet to ${networkName}.`,
        );
      }
      ...
      return {
        success: true,
        accountInfo: info,
        walletApi: (await window.cardano[
          walletName
        ].enable()) as CardanoApiWallet,
      };
    } catch (error) {
      const errorMessage =
      ...
      return {
        ...
        error:
          errorMessage === 'undefined'
            ? 'Unknown wallet error (connection rejected or timed out)'
            : errorMessage,
      };
    }
  }
  ...
  private async fetchAccountInfo(params?: any): Promise<AccountInfo> {
    ...
    try {
      ...
      console.error(
        `[MeshProvider] Failed to convert change address: ${rawAddress}`,
        e,
      );
    ...
    try {
      ...
      console.error(
        `[MeshProvider] Failed to convert stake address: ${stakeKeyHex}`,
        e,
      );
    ...
    const dRepIDBech32 = dRepKey ? await this.getDRepIdentifier(walletApi) : '';

    const registeredStakeKeys =
      (await walletApi.cip95?.getRegisteredPubStakeKeys()) || [];
    ...
    return {
      ...
      dRepInfo: {
        ...
        dRepId:
          dRepIDBech32 && dRepIDBech32.includes('1')
            ? fromBech32ToHex(dRepIDBech32)
            : null,
  ...
  private async getDRepIdentifier(

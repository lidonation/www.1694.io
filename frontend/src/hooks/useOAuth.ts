...
export const useGetOAuthProvider = (
  ...
  return useQuery(
    ...
    {
      enabled: !!stakeKeyBech32 && !!provider,
    }
  );
};
...
export const useCreateOAuth = () => {
  ...
  return useMutation(
    (payload: CreateOAuthDto) => createOAuth(payload),
    {
      onSuccess: (_, variables) => {
        queryClient.invalidateQueries(OAUTH_KEYS.providers(variables.stakeKeyBech32));
...
export const useUpdateOAuth = () => {
  ...
  return useMutation(
    (payload: { data: UpdateOAuthDto; stakeKeyBech32: string; providerId: number }) => 
      updateOAuth(payload.data),
    {
      onSuccess: (_, variables) => {
        ...
        if (variables.data.provider) {
          queryClient.invalidateQueries(
            OAUTH_KEYS.provider(variables.stakeKeyBech32, variables.data.provider)
          );
        }
      },
    }
...
export const useDeleteOAuth = () => {
  ...
  return useMutation(
    ({ stakeKeyBech32, providerId }: { stakeKeyBech32: string; providerId: number }) =>

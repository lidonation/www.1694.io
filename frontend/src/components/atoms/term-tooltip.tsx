...
const GLOSSARY_TERMS: Record<string, string> = {
  ...
  'Constitutional Committee':
    'A body that ensures governance actions follow the Cardano Constitution.',
  'Governance Action':
    'An on-chain proposal that can change the protocol or the constitution.',
  Ratification:
    'The process by which a governance action is approved by the required bodies.',
  Enactment: 'The final execution of a ratified governance action on-chain.',
  Epoch:
    'A period of time in the Cardano network (typically 5 days on mainnet).',
  UTxO: "Unspent Transaction Output, the fundamental unit of Cardano's ledger.",
  'CIP-1694':
    'The Cardano Improvement Proposal that describes the on-chain governance system.',
  Voltaire: 'The final era of Cardano, focusing on decentralized governance.',
  'Guardrails Script':
    'A script that enforces specific rules and conditions for governance actions.',
  Ada: 'The native cryptocurrency of the Cardano blockchain.',
  Threshold:
    'The minimum percentage of votes required to ratify a governance action.',
};
...
export const TermTooltip = ({
  ...
  return (
    <Tooltip
      ...
      placement="top"
    >
      <span className="border-primary-400 hover:text-primary-600 inline-flex cursor-help items-center border-b border-dotted transition-colors">
        {children || term}
        {showIcon && (
          <InfoOutlinedIcon sx={{ fontSize: 14, ml: 0.5, opacity: 0.6 }} />
        )}
      </span>
    </Tooltip>
  );

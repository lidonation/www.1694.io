      ...
      >
        {!isAtLatestPoint && onLoadNewer && (
          <TimelineItem sx={{ minHeight: 'auto' }}>
            <TimelineSeparator
              sx={{ width: '30px', flexShrink: 0, alignItems: 'center' }}
            >
              <TimelineDot
                variant="outlined"
                color="primary"
              ...
              <TimelineConnector
                sx={{
                  ...
                  backgroundImage:
                    'linear-gradient(to bottom, #E5E7EB 50%, rgba(255,255,255,0) 0%)',
            ...
            <TimelineContent sx={{ py: '6px', px: 1 }}>
              <button
                className="rounded-full border border-orange-100/50 bg-orange-50/50 px-3 py-1 text-[10px] font-bold tracking-wider text-orange-500 uppercase"
                onClick={onLoadNewer}
              >
                Load Newer
              </button>
          ...
          <React.Fragment key={epoch.epochNo}>
            ...
            <TimelineItem sx={{ minHeight: 'auto' }}>
              ...
              >
                {epochIndex === 0 && isAtLatestPoint ? (
                  ...
                  <TimelineConnector
                    sx={{
                      ...
                      backgroundColor: 'transparent',
                      backgroundImage:
                        'linear-gradient(to bottom, #D1D5DB 50%, rgba(255,255,255,0) 0%)',
                      backgroundPosition: 'center',
                      backgroundSize: '1.5px 8px',
                      backgroundRepeat: 'repeat-y',
                    }}
                ...
                >
                  <div className="border-primary ring-primary/10 h-3 w-3 rounded-full border-2 bg-white ring-4" />
                </TimelineDot>
                <TimelineConnector
                  sx={{
                    width: '1.5px',
            ...
            {epoch.items.map((item: any) => (
              <TimelineItem key={item.id} sx={{ minHeight: 'auto' }}>
                ...
                >
                  ...
                  >
                    <div
                      className={`h-2 w-2 rounded-full border ${item.type === 'bundled_delegations' ? 'bg-primary border-primary' : 'border-gray-300 bg-white'}`}
                    />
                  </TimelineDot>
                  <TimelineConnector
                    sx={{
                  ...
                  <Box sx={{ width: '100%' }}>
                    ...
                    {item.type === 'registration' && (
                      <Link
                        ...
                        target="_blank"
                      >
                        <div className="w-fit rounded-full border border-gray-100 bg-gray-50 px-3 py-1 text-[10px] font-bold text-gray-500 shadow-sm">

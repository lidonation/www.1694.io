...
const committeeOptions: FilterOption[] = [
  ...
  {
    label: 'Membership & Community Committee',
    value: 'Membership & Community Committee',
  },
  { label: 'Product Committee', value: 'Product Committee' },
  { label: 'Civics Committee', value: 'Civics Committee' },
  {
    label: 'Intersect Steering Committee Election',
    value: 'Intersect Steering Committee',
  },
  { label: 'Intersect Board ', value: 'Intersect Board' },
  {
    label: 'Growth and Marketing Committee',
    value: 'Growth and Marketing Committee',
  },
...
const ProposalFilter: React.FC<ProposalFilterProps> = ({
  ...
  useEffect(() => {
    ...
    const {
      ...
      committees: storedCommittees = [],
    } = stored;

    const needsApply =
      (!searchParams.has('categories') && storedCategories.length) ||
      (!searchParams.has('committees') && storedCommittees.length);

    if (needsApply) {
      if (!searchParams.has('categories'))
        params.set('categories', storedCategories.join(','));
      if (!searchParams.has('committees'))
        params.set('committees', storedCommittees.join(','));
      params.set('page', '1');
      router.push(`?${params.toString()}`);
    }
  }, [searchParams]);

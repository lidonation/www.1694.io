...
export class IpfsService {
  ...
  constructor(
    ...
    const kuboGatewayUrl = this.configService.get<string>(
      'IPFS_KUBO_GATEWAY_URL',
    );

    this.kubo = kuboApiUrl
      ? {
          apiUrl: kuboApiUrl,
          gatewayUrl: kuboGatewayUrl || kuboApiUrl.replace(':5001', ':8080'),
        }
      : null;
  ...
  private async kuboPost<T>(
    ...
    const res = await lastValueFrom(
      this.httpService.post<T>(url.toString(), data),
    );
    return res.data;
  }
  ...
  private async bfGet<T>(
    cfg: BlockfrostIPFSConfig,
    endpoint: string,
  ): Promise<T> {
    const res = await lastValueFrom(
      this.httpService.get<T>(`${cfg.url}${endpoint}`, {
        headers: this.bfHeaders(cfg),
      }),
    );
    return res.data;
  }

  private async bfPost<T>(
    cfg: BlockfrostIPFSConfig,
    endpoint: string,
    data?: any,
  ): Promise<T> {
    const res = await lastValueFrom(
      this.httpService.post<T>(`${cfg.url}${endpoint}`, data, {
        headers: this.bfHeaders(cfg),
      }),
    );
    return res.data;
  ...
  async getIPFSContent(ipfsHash: string): Promise<any> {
    return this.withChain([
              ...
              fn: () =>
                lastValueFrom(
                  this.httpService.get(
                    `${this.kubo.gatewayUrl}/ipfs/${ipfsHash}`,
                  ),
                ).then((r) => r.data),
            },
          ]
        : []),
      {
        label: 'bf-primary',
        fn: () =>
          this.bfGet(this.blockfrostPrimary, `/ipfs/gateway/${ipfsHash}`),
      },
      {
        label: 'bf-fallback',
        fn: () =>
          this.bfGet(this.blockfrostFallback, `/ipfs/gateway/${ipfsHash}`),
      },
    ]);
  }
  ...
  async checkPinStatus(hash: string): Promise<IPFSPinStatusResponse> {
    return this.withChain<IPFSPinStatusResponse>([
        ...
        ? [
            {
              ...
              fn: async () => {
                ...
                return {
                  ...
                  state: (entry
                    ? 'pinned'
                    : 'unpinned') as IPFSPinStatusResponse['state'],
                  size: '0',
                  time_created: 0,
                  time_pinned: 0,
                };
        ...
        : []),
      {
        label: 'bf-primary',
        fn: () =>
          this.bfGet<IPFSPinStatusResponse>(
            this.blockfrostPrimary,
            `/ipfs/pin/list/${hash}`,
          ),
      },
  ...
  async getAttachmentFromIPFS(hash: string, res: Response): Promise<any> {
    const streamAttempts = [
      ...(this.kubo
        ? [
            {
              label: 'kubo-gateway',
              url: `${this.kubo.gatewayUrl}/ipfs/${hash}`,
            },
          ]
        : []),
      {
        label: 'bf-primary',
        url: `${this.blockfrostPrimary.url}/ipfs/gateway/${hash}`,
        headers: this.bfHeaders(this.blockfrostPrimary),
      },
      {
        label: 'bf-fallback',
        url: `${this.blockfrostFallback.url}/ipfs/gateway/${hash}`,
        headers: this.bfHeaders(this.blockfrostFallback),
      },
    ];
    ...
    for (const attempt of streamAttempts) {
      try {
        ...
        this.logger.warn(
          `IPFS stream [${attempt.label}] failed for ${hash}: ${err.message}`,
        );
      }
    }

    throw new HttpException(
      'Failed to stream IPFS content',
